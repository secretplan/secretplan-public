using ExTween;
using Godot;
using SokoCore;
using SokoGame.Animation;
using SokoGame.World;

namespace SokoGame.Transforms;

public readonly record struct TransformMoveCardinal(EntityId EntityId, CardinalDirection Direction)
    : ITransform
{
    private readonly float _overReachPercent = 0.15f;

    public Frame ApplyTo(Frame frame)
    {
        var entity = frame.GetEntity(EntityId);
        frame.SetEntity(EntityId, entity with { Position = entity.Position + Direction });
        return frame;
    }

    public void BuildBeforeAnimation(MultiplexTween tween, EntityViewTable table)
    {
        var offset = Offset.FromDirection(Direction);
        var directionOffsetAsVector = new Vector2(offset.X, offset.Y);
        var entity = table.GetEntity(EntityId);
        tween
            .Add(entity.TweenablePositionOffsetPercent.TweenTo(
                directionOffsetAsVector * 0.25f, 0.03f, Ease.QuadFastSlow))
            ;
    }

    public void BuildAfterAnimation(MultiplexTween tween, EntityViewTable table)
    {
        var offset = Offset.FromDirection(Direction);
        var directionOffsetAsVector = new Vector2(offset.X, offset.Y);
        var entity = table.GetEntity(EntityId);
        tween
            .Add(new SequenceTween()
                .Add(entity.TweenableAngleDegrees.CallbackSetTo(0))
                .Add(entity.TweenablePositionOffsetPercent.CallbackSetTo(directionOffsetAsVector * _overReachPercent))
                .Add(entity.TweenablePositionOffsetPercent.TweenTo(Vector2.Zero, 0.06f, Ease.QuadFastSlow))
            );
    }

    public override string ToString()
    {
        return $"MOVE {EntityId} {Direction}";
    }
}